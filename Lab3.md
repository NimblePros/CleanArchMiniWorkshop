...
```csharp
using Ardalis.Result;
using CleanArchWebShop.Core.CatalogAggregate;
  ...
  : IRequestHandler<PlaceOrderCommand, Result<int>>
{
  public async Task<Result<int>> Handle(PlaceOrderCommand request, CancellationToken cancellationToken)
  {
    ...
    var requestedItems = items.Where(i => itemIds.Contains(i.Id)).ToList();

    if (requestedItems.Count != itemIds.Count)
    {
      var missingIds = itemIds.Except(requestedItems.Select(i => i.Id));
      return Result<int>.NotFound($"Items not found: {string.Join(", ", missingIds)}");
    }
    ...
    {
      var item = requestedItems.First(i => i.Id == requestItem.ItemId);

      // Business Rule: Check stock availability
      if (item.StockQuantity < requestItem.Quantity)
      {
        return Result<int>.Error(
          $"Insufficient stock for {item.Name}. Available: {item.StockQuantity}, Requested: {requestItem.Quantity}"
        );
      }

      // Add to order
      order.AddItem(item.Id, item.Name, requestItem.Quantity, item.Price);

      // Reduce stock
      item.ReduceStock(requestItem.Quantity);
...
3. Create order
4. Add items and reduce stock atomically
5. Complete order
6. Save to database
7. Return order ID or error

**Discussion:** Notice how business rules are split between entities (Order, Item) and the handler (orchestration). This is intentional!
...
public class OrderConfiguration : IEntityTypeConfiguration<Order>
{
  public void Configure(EntityTypeBuilder<Order> builder)
  {
    ...
    // Configure the OrderItems collection
...
Create `Infrastructure/Data/Config/OrderItemConfiguration.cs`:
...
**Key Points:**
- Order owns the OrderItems collection
- Cascade delete: if Order is deleted, OrderItems are too
- Indexes on common query fields (UserId, OrderDate)
...
Let's expose the feature through an API.

### Create OrdersController

In `CleanArchWebShop.Web/Api`, create `OrdersController.cs`:
...
dotnet ef migrations add AddOrders -s CleanArchWebShop.Web.csproj -p ..\CleanArchWebShop.Infrastructure\CleanArchWebShop.Infrastructure.csproj -o Data/Migrations
...
2. **First, get some item IDs** (from Lab 2):

```powershell
curl https://localhost:5001/api/catalog/items
```

Note the IDs of a few items.

3. **Place an order** using curl or Postman:
...
And status code `201 Created` with a Location header.

5. **Test error scenarios:**

```powershell
# Order with invalid item ID
curl -X POST https://localhost:5001/api/orders `
  -H "Content-Type: application/json" `
  -d '{
    "userId": "user123",
...
```powershell
# Order with quantity exceeding stock
curl -X POST https://localhost:5001/api/orders `
  -H "Content-Type: application/json" `
  -d '{
    "userId": "user123",
...
Should return `400 Bad Request` with stock error message.
...
The template should have created test projects. We'll use `CleanArchWebShop.UnitTests`.

### Create Order Tests

In `CleanArchWebShop.UnitTests`, create `Core/OrderAggregate/OrderTests.cs`:
...
public class PlaceOrderCommandHandlerTests
{
  ...
  [Fact]
  public async Task Handle_ValidOrder_CreatesOrderAndReducesStock()
  {
    // Arrange
    var items = new List<Item>
    {
      new Item("Laptop", 999.99m, 10) { Id = 1 },
      new Item("Mouse", 29.99m, 50) { Id = 2 }
    };
    ...
    Assert.Equal(8, items[0].StockQuantity); // Laptop: 10 - 2
    Assert.Equal(49, items[1].StockQuantity); // Mouse: 50 - 1
  }
  ...
  public async Task Handle_InsufficientStock_ReturnsError()
  {
    ...
    var items = new List<Item>
    {
      new Item("Laptop", 999.99m, 1) { Id = 1 }
    };
    ...
    var command = new PlaceOrderCommand(
      ...
      "Standard",
      "Credit Card",
      new List<OrderItemRequest> { new(1, 5) } // Request more than available
    );
    ...
    Assert.Contains("Insufficient stock", result.Errors[0]);
    _orderRepositoryMock.Verify(
      r => r.AddAsync(It.IsAny<Order>(), It.IsAny<CancellationToken>()),
      Times.Never
    );
  ...
  public async Task Handle_ItemNotFound_ReturnsNotFound()
  {
    ...
    var command = new PlaceOrderCommand(
      ...
      "Standard",
      "Credit Card",
      new List<OrderItemRequest> { new(999, 1) } // Non-existent item
    );
...
- ✅ Configured EF Core relationships
- ✅ Created an API endpoint for placing orders
- ✅ Tested the feature end-to-end
- ✅ Written comprehensive unit tests
- ✅ Validated error handling (insufficient stock, missing items)
...
| **Stock Management** | Direct SQL, file operations | Entity method |
